        ...
        if result ~= nil then
            ...
            if AllJob[table.name] ~= nil then
                if AllJob[table.name].grade_table[tostring(table.grade)] then
                    DubleJob[xPlayer.identifier] = AllJob[table.name].grade_table[tostring(table.grade)] 
                    xPlayer.triggerEvent("mxm_dublejob:updateJob",DubleJob[xPlayer.identifier])
                end
            end
        end
    end)
...
exports('getJob',function(source)
    ...
    if xPlayer then
        if DubleJob[xPlayer.identifier] ~= nil then
            return DubleJob[xPlayer.identifier]
        end 
    end
end)
...
--[[
  local setJob = exports["mxm_dublejob"]:setJob(src,job,grade)
    ...
    if xPlayer then
        if AllJob[job] ~= nil then
            if AllJob[job].grade_table[grade] then
                DubleJob[xPlayer.identifier] = AllJob[job].grade_table[grade]
                xPlayer.triggerEvent("mxm_dublejob:updateJob", DubleJob[xPlayer.identifier])
                return "set"
            else
                return "no_grade"
...
AddEventHandler('playerDropped', function()
	...
	if xPlayer then
        if DubleJob[xPlayer.identifier] ~= nil then 
            local JobTable = {
                name = DubleJob[xPlayer.identifier].name,
                grade = DubleJob[xPlayer.identifier].grade
            }
            MySQL.Async.execute('UPDATE users SET job_2 = @job_2 WHERE identifier = @identifier', {
                ['@job_2'] = json.encode(JobTable), 
                ["@identifier"] = xPlayer.identifier
            }, function(c2)
                DubleJob[xPlayer.identifier] = nil
            end)
        end 
	end
end)
        ...
        if tonumber(args[1]) then
            if args[2] then
                if args[3]  then
                    ...
                    if xPlayer then
                        local setJob = exports["mxm_dublejob"]:setJob(tonumber(args[1]),args[2],args[3])
                        if setJob == "set" then
	...
	function payCheck()
        for a, getplayers in pairs(GetPlayers()) do   
            if tonumber(getplayers) ~= nil then
                ...
                if xPlayer then
                    local getJob = exports["mxm_dublejob"]:setJob(getplayers)
